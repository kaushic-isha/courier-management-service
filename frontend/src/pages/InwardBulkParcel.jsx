...
export default function InwardBulkParcel() {
  const navigate = useNavigate();
  const user = JSON.parse(localStorage.getItem("loggedInUser") || "null");
  const guard = GuardAdminOrStaff({ user });
  if (guard) return guard;

  const [form, setForm] = useState({ parcels: "", weight: "", remarks: "" });
        ...
        <section className="bulk-page-card">
          <div className="bulk-form-grid">
            <input
              ...
              placeholder="Parcels"
              value={form.parcels}
              onChange={(e) => setForm((prev) => ({ ...prev, parcels: e.target.value }))}
            />
            <input
              className="inward-input"
              type="text"
